...
| スマホモックアップ枠（ホーム / ビンゴカード / 会場マップ。#89 でアワード投票を出す場合はアワード投票画面も） | 4-1-1 | `onboarding/award-screen-*.png`（受領後 `onboarding/mockup-home.png` / `mockup-bingo-card.png` / `mockup-venue-map.png` / `mockup-award.png` へ） | オンボーディングの全スライドにアワード投票の画面が写っている。説明文と中身が食い違う |
