...
前版で「要判断」としていた項目は、2026-08-26 にすべて決定した。
**この仕様に未決定の項目は残っていない。**
...
| メニュー画面 | 仕様書 p.7 のナビ4番目のタブ。**削除**（`icon/action/menu.png` は未使用のまま残す） |
| 配色の検証 | **自動テストで担保。** `tests/unit/design-tokens.test.ts` |
| `--pf-ink-muted` | AA 基準未達のため `#8A8070` → **`#6E6656`** へ補正 |
