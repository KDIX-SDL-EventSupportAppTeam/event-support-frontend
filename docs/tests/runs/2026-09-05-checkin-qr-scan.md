...
- `src/features/booth/pages/BoothListPage/BoothListPage.tsx`（モーダルの「チェックイン」の遷移先を `/checkin?booth_id=…` から `/checkin` へ。設計書 §4 の8ファイルに対する追加）
...
| L-6 | `/booth-list` のモーダルの「チェックイン」を押すと `/checkin`（QR 読み取り画面）へ行く。`?booth_id=` は付かない | 合格。遷移後の URL は `/checkin`、「QRコードをかざしてください」が表示された |
| L-7 | scan 画面のフォールバック導線が、開いた直後は出ず、8 秒後に出る | 合格。直後は `.checkin-qr-fallback-link` が存在せず、9 秒後に「ブース一覧から選ぶ」が現れた |
...
- **`/booth-list` からの迂回路を塞いだ。** ブース一覧モーダルの「チェックイン」は `/checkin?booth_id=<uuid>` へ
  遷移していたため、カメラを起動せずに任意のブースへチェックインできた。issue #84 の目的がそのまま残るため、
  遷移先を `/checkin`（QR 読み取り画面）に変更した。設計書 §4 が挙げた8ファイルの外だが、
  I1（チェックイン＝実訪問）に直結するため徳山さんの判断で今回の PR に含めた。
- **フォールバック導線は「エラー時＋8秒経過後」表示にした。** issue #84 は「カメラが使えない / 権限拒否 /
  対象外の QR のときだけ」出すよう求めており、常時表示だと QR の関門が実質かからない。
  一方 `getUserMedia` は端末によって成功も失敗も返さず保留になることがあり
  （2026-09-05 のヘッドレス確認で実際に起きた）、エラー時だけに絞ると先に進めなくなる。両方を満たす形にした。
