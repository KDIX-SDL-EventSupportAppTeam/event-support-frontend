 ...
 * - `パスワードを忘れた場合」` は issue #107（自分で再設定できる文面に戻した）で除外
 * - `アワード` は issue #89（今年アプリ内で実施すると決定）で除外
 * 去年の日程・運用の禁止語（10/10・DAY1・スタッフ立会い等）は引き続き残す。
 */
const FORBIDDEN = ['10/10', '10/11', 'DAY1', 'DAY2', '2025', 'スタッフに見せ', 'スタッフにコイン', 'ガチャポンブース']
